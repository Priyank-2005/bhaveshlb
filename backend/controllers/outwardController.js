...
/**
 * CREATE OUTWARD ENTRY: Header + Line Items (requires transaction)
...
exports.createOutwardEntry = async (req, res) => {
    ...
    try {
        ...
        // 1. Insert into outward_entries (Header)
        ...
        // 2. Prepare line items for bulk insert (including Lot No. & Godown No.)
        ...
        // 3. Insert into outward_line_items (Details)
        await connection.query(
            'INSERT INTO outward_line_items (entry_id, item_id, unit_of_measure, quantity, lot_no, godown_no) VALUES ?',
            [lineItemValues]
        );

        // 3. --- NEW: UPDATE STOCK BALANCE ---
        for (const item of line_items) {
            const { item_id, quantity } = item;
            
            // Note the subtraction: quantity - VALUES(quantity)
            ...
            // OPTIONAL: Add a check here to ensure stock doesn't go below zero (negative inventory check)
        }
        ...
        await connection.rollback(); // Undo everything if any step failed
...
/**
 * GET ALL OUTWARD ENTRIES (Summary View for table list)
...
/**
 * GET SINGLE OUTWARD ENTRY WITH DETAILS
...
exports.getOutwardEntryDetails = async (req, res) => {
    ...
    try {
        ...
        const [lineItems] = await db.execute(
            'SELECT l.*, m.item_name FROM outward_line_items l JOIN item_master m ON l.item_id = m.item_id WHERE l.entry_id = ?',
            [id]
        );
...
exports.updateOutwardEntry = async (req, res) => {
    ...
    try {
        ...
        // 1b. Reverse the inventory impact of the old entry (Outward originally subtracted, so we ADD it back)
        ...
        // 4. APPLY NEW STOCK IMPACT (Outward means subtracting the NEW quantity)
        for (const item of line_items) {
            const updateStockQuery = `
                INSERT INTO inventory_stock (item_id, quantity)
                VALUES (?, ?)
