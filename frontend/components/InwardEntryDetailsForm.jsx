// frontend/components/InwardEntryDetailsForm.jsx
...
export default function InwardEntryDetailsForm({ entryId, setViewMode, setEntryToViewId }) {
    ...
    useEffect(() => {
        ...
        const fetchEntryDetails = async () => {
            try {
                // Fetch the specific entry details from the backend
                const response = await API.get(`/inward/${entryId}`);
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        try {
            await API.put(`/inward/${entryId}`, payload);
            ...
            <form onSubmit={handleUpdateSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4 border p-4 rounded-md bg-gray-50">
                    <div>
                        ...
                        <input type="date" name="inward_date" value={entryData.inward_date.substring(0, 10)} onChange={handleHeaderChange} className="mt-1 block w-full border rounded-md p-2 bg-white" required />
                    </div>
                    <div>
